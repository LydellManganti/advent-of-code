...
func main() {
	...
	blueLocation := []Position{Position{0, 0, 0}}
	redLocation := []Position{Position{0, 0, 0}}

	updatePosition(blueWire, &blueLocation)
	updatePosition(redWire, &redLocation)

	crossings := getCrossings(blueLocation, redLocation)

	minimumSteps := []int{}
	for _, crossing := range crossings {
		fmt.Printf("X:%v, Y:%v, Total Steps:%v\n", crossing.X, crossing.Y, crossing.Steps)
		minimumSteps = append(minimumSteps, crossing.Steps)
	}

	sort.Ints(minimumSteps)
	fmt.Printf("%v\n", minimumSteps[1])
}

func updatePosition(wire Wire, location *[]Position) {
	for _, wireInstruction := range wire {
		dir, step := getDirection(wireInstruction)
		moveDirection := (*location)[len(*location)-1]
		switch dir {
		case "U":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X + i, moveDirection.Y, moveDirection.Steps + i})
			}
		case "D":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X - i, moveDirection.Y, moveDirection.Steps + i})
		...
		case "R":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X, moveDirection.Y + i, moveDirection.Steps + i})
			}
		case "L":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X, moveDirection.Y - i, moveDirection.Steps + i})
...
func getCrossings(blueLocation []Position, redLocation []Position) []Position {
	var crossingPosition = []Position{}
	for _, blue := range blueLocation {
		for _, red := range redLocation {
			if blue.X == red.X && blue.Y == red.Y {
				fmt.Printf("Crossing detected X:%v, Y:%v, BlueSteps:%v, RedSteps:%v, TotalSteps:%v\n", blue.X, blue.Y, blue.Steps, red.Steps, blue.Steps+red.Steps)
				pos := Position{blue.X, red.Y, blue.Steps + red.Steps}
				crossingPosition = append(crossingPosition, pos)
			}
		}
