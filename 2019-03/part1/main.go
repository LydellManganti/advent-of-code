...
func main() {
	...
	blueLocation := []Position{Position{0, 0}}
	redLocation := []Position{Position{0, 0}}
	...
	for _, crossing := range crossings {
		fmt.Printf("X:%v, Y:%v, Total Steps:%v\n", crossing.X, crossing.Y, Abs(crossing.X)+Abs(crossing.Y))
		minimumSteps = append(minimumSteps, Abs(crossing.X)+Abs(crossing.Y))
	}
...
func updatePosition(wire Wire, location *[]Position) {
	for _, wireInstruction := range wire {
		...
		case "U":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X + i, moveDirection.Y})
			}
		case "D":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X - i, moveDirection.Y})
		...
		case "R":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X, moveDirection.Y + i})
			}
		case "L":
			for i := 1; i <= step; i++ {
				*location = append(*location, Position{moveDirection.X, moveDirection.Y - i})
...
func getCrossings(blueLocation []Position, redLocation []Position) []Position {
	...
	for _, blue := range blueLocation {
		for _, red := range redLocation {
			if blue == red {
				fmt.Printf("Crossing detected X:%v, Y:%v\n", blue.X, blue.Y)
				crossingPosition = append(crossingPosition, blue)
			}
		}
